...
class Frame:
    ...
    def __init__(
        self, width, height, spacing=0.5, border_color="white", shadow_alpha=0.4
    ):
        """
    ...
    def render_to_pil(
        ...
        vertices = self.calculate_vertices(
            x_center, y_center, scaled_width, scaled_height
        )
        ...
        vertices_px[:, 1] = canvas_height - (
            vertices_px[:, 1] * pixels_per_unit
        )  # Flip Y
        ...
        shadow_vertices_px[:, 1] += (
            shadow_offset_px  # Shadow offset down (in PIL: Y increases downward)
        )
        ...
        if len(border_points) >= 2:
            for i in range(len(border_points)):
                ...
                border_draw.line(
                    [p1, p2], fill=border_color_rgb + (255,), width=border_width
                )
...
def create_frame_from_preset(preset):
    ...
    if shape_type == "parallelogram":
        ...
        return ParallelogramFrame(
            width, height, spacing, skew_angle, border_color, shadow_alpha
        )
    elif shape_type == "rhombus":
        rotation_angle = preset.get("rotation_angle", 0)
        return RhombusFrame(
            width, height, spacing, rotation_angle, border_color, shadow_alpha
        )
