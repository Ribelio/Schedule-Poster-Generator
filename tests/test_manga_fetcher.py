...
def test_get_manga_id_success(fetcher):
    ...
    mock_response.json.return_value = {
        "result": "ok",
        "data": [
            {"id": "test-manga-id", "attributes": {"title": {"en": "Test Manga"}}}
        ],
    }
...
def test_get_volume_covers(fetcher):
    ...
    mock_response.json.return_value = {
        "result": "ok",
        "data": [
            {"attributes": {"volume": "1", "fileName": "vol1.jpg"}},
            {"attributes": {"volume": "2", "fileName": "vol2.jpg"}},
        ],
    }
...
def test_get_volume_covers_partial(fetcher):
    ...
    mock_response.json.return_value = {
        "result": "ok",
        "data": [{"attributes": {"volume": "1", "fileName": "vol1.jpg"}}],
    }
