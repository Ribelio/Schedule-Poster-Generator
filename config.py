...
class PosterConfig:
    ...
    zoom_factor: float = (
        1.1  # Center-crop-zoom factor: Higher values = more zoom into center
    )
    vertical_offset: float = (
        -0.10
    )  # Vertical offset for image positioning (positive = higher)
    ...
    background_lineart_alpha: float = (
        0.15  # Transparency (0.0 = fully transparent, 1.0 = opaque)
    )
    background_lineart_color: str = (
        "white"  # Color for the line art (if using monochrome)
    )
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "PosterConfig":
        ...
        if (
            "stagger_preset" in config_data
            and config_data["stagger_preset"] is not None
        ):
