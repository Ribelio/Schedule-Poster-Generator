...
from PySide6.QtWidgets import (
    ...
    QLabel, QPushButton, QHBoxLayout, QDoubleSpinBox, 
    QSpinBox, QLineEdit, QComboBox, QColorDialog, QCheckBox, QFileDialog
)
...
class SettingsPanel(QWidget):
    ...
    def _create_form_widgets(self):
        ...
        self.schedule_widget.fetchCoversRequested.connect(self.fetchCoversRequested.emit)
        ...
        self._add_float_widget("horizontal_padding", "Horizontal Padding", 0.0, 5.0, 0.1)
        self._add_float_widget("column_spacing", "Column Spacing", 0.0, 5.0, 0.1)
        
        # Shape
        self.form_layout.addRow(QLabel("<b>Frame Shape</b>"))
        self._add_combo_widget("shape_type", "Shape Type", ["parallelogram", "rhombus", "rectangle", "hexagon"], "parallelogram")
        ...
        self._add_float_widget("shape_rotation_angle", "Rotation Angle", -180.0, 180.0, 1.0)
        
        # Stagger
        self.form_layout.addRow(QLabel("<b>Stagger</b>"))
        self._add_combo_widget("stagger_type", "Stagger Type", ["none", "alternating", "wave"], "none")
        ...
        self._add_combo_widget("title_fontweight", "Title Weight", ["normal", "bold"], "bold")
        ...
        self._add_color_widget("frame_border_color", "Frame Border Color") # Note: legacy naming in config?
        ...
        self._add_float_widget("background_lineart_alpha", "Line Art Alpha", 0.0, 1.0, 0.05)
    ...
    def _add_float_widget(self, name, label, min_val, max_val, step):
        ...
        widget.setValue(getattr(self.config, name, 0) if not name.startswith("shape_") and not name.startswith("stagger_") else 0)
    ...
    def _add_color_widget(self, name, label):
        ...
        if name.startswith("shape_"): val = self.config.shape_preset.get(name.replace("shape_", ""), "#ffffff") # Simplified logic for now
        widget.setText(val)
        widget.textChanged.connect(self.configChanged.emit)
    ...
    def _pick_file(self, line_edit):
        path, _ = QFileDialog.getOpenFileName(self, "Select File", "", "Images (*.png *.jpg *.jpeg)")
        if path:
    ...
    def _update_widgets_from_config_obj(self):
        ...
        # Simple fields
        if 'manga_title' in w: w['manga_title'].setText(c.manga_title)
        if 'zoom_factor' in w: w['zoom_factor'].setValue(c.zoom_factor)
        if 'vertical_offset' in w: w['vertical_offset'].setValue(c.vertical_offset)
        if 'cols' in w: w['cols'].setValue(c.cols)
        if 'title_row_height' in w: w['title_row_height'].setValue(c.title_row_height)
        if 'vertical_padding' in w: w['vertical_padding'].setValue(c.vertical_padding)
        if 'bottom_margin' in w: w['bottom_margin'].setValue(c.bottom_margin)
        if 'horizontal_padding' in w: w['horizontal_padding'].setValue(c.horizontal_padding)
        if 'column_spacing' in w: w['column_spacing'].setValue(c.column_spacing)
        
        # Title
        if 'title_fontsize' in w: w['title_fontsize'].setValue(c.title_fontsize)
        if 'title_fontweight' in w: w['title_fontweight'].setCurrentText(c.title_fontweight)
        if 'title_color' in w: w['title_color'].setText(c.title_color)
        if 'title_fontfamily' in w: w['title_fontfamily'].setText(c.title_fontfamily)
        
        # Text
        if 'date_fontsize' in w: w['date_fontsize'].setValue(c.date_fontsize)
        if 'volume_fontsize' in w: w['volume_fontsize'].setValue(c.volume_fontsize)
        if 'text_color' in w: w['text_color'].setText(c.text_color)
        
        # Background
        if 'background_color' in w: w['background_color'].setText(c.background_color)
        if 'frame_border_color' in w: w['frame_border_color'].setText(c.frame_border_color)
        
        # Lineart
        if 'background_lineart_enabled' in w: w['background_lineart_enabled'].setChecked(c.background_lineart_enabled)
        if 'background_lineart_path' in w: w['background_lineart_path'].setText(c.background_lineart_path)
        if 'background_lineart_alpha' in w: w['background_lineart_alpha'].setValue(c.background_lineart_alpha)
        
        # Output
        if 'output_dir' in w: w['output_dir'].setText(c.output_dir)
        if 'dpi' in w: w['dpi'].setValue(c.dpi)
        
        # Shape preset
        if 'shape_type' in w: w['shape_type'].setCurrentText(c.shape_preset.get('type', 'parallelogram'))
        if 'shape_width' in w: w['shape_width'].setValue(c.shape_preset.get('width', 1.5))
        if 'shape_height' in w: w['shape_height'].setValue(c.shape_preset.get('height', 2.5))
        if 'shape_spacing' in w: w['shape_spacing'].setValue(c.shape_preset.get('spacing', 0.0))
        if 'shape_border_color' in w: w['shape_border_color'].setText(c.shape_preset.get('border_color', 'gold'))
        if 'shape_shadow_alpha' in w: w['shape_shadow_alpha'].setValue(c.shape_preset.get('shadow_alpha', 0.4))
        if 'shape_skew_angle' in w: w['shape_skew_angle'].setValue(c.shape_preset.get('skew_angle', -15))
        if 'shape_rotation_angle' in w: w['shape_rotation_angle'].setValue(c.shape_preset.get('rotation_angle', 0))
        
        # Stagger preset
        if 'stagger_type' in w: w['stagger_type'].setCurrentText(c.stagger_preset.get('type', 'none'))
        if 'stagger_offset' in w: w['stagger_offset'].setValue(c.stagger_preset.get('offset', 0.1))
