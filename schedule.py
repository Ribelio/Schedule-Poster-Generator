...
from config import (
    ...
    cover_urls as default_cover_urls,
...
def load_config_from_json(json_path: str):
    ...
    if "config" in data:
        ...
        schedule = (
            [tuple(item) for item in data["schedule"]]
            if "schedule" in data
            else default_schedule
        )
        cover_urls = (
            {int(k): v for k, v in data["cover_urls"].items()}
            if "cover_urls" in data
            else default_cover_urls
        )
...
def main():
    ...
    parser.add_argument(
        "-o", "--output", type=str, help="Output path for the generated poster"
    )
