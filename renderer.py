...
def calculate_scale_factor(num_vols, frame_width, frame_spacing):
    ...
    if num_vols > 0:
        ...
        scale_factor = (
            reference_width / total_unscaled_width if total_unscaled_width > 0 else 1.0
        )
...
def calculate_layout_dimensions(schedule, config):
    ...
    cell_height = (
        total_content_height - (content_rows - 1) * config.vertical_padding
    ) / content_rows
    ...
    for dx in range(-outline_width, outline_width + 1):
        for dy in range(-outline_width, outline_width + 1):
            if dx * dx + dy * dy <= outline_width * outline_width:
                draw.text(
                    (x + dx, y + dy), text, font=font, fill=outline_color + (255,)
                )
...
def render_text_layer(
    ...
    if outline_rgb:
        draw_text_with_outline(
            draw, text, (x_px, y_px), font, fill_rgb, outline_rgb, outline_width
        )
    else:
        ...
        text_height = bbox[3] - bbox[1]
        draw.text(
            (x_px - text_width / 2, y_px - text_height / 2),
            text,
            font=font,
            fill=fill_rgb + (255,),
        )
...
def render_background_lineart_layer(
    canvas_width: int, canvas_height: int, config, pixels_per_unit: float
    ...
    if not os.path.exists(config.background_lineart_path):
        print(
            f"Warning: Background line art not found at {config.background_lineart_path}"
        )
        return None
...
def render_volume_image_layer(
    ...
    canvas_img = Image.new("RGBA", (canvas_width, canvas_height), (0, 0, 0, 0))
    canvas_img.paste(
        img_resized, (x_px, y_px), img_resized if img_resized.mode == "RGBA" else None
    )
...
def composite_flat(
    canvas_width: int, canvas_height: int, layers: List[Image.Image]
) -> Image.Image:
    """
...
def render_poster_to_buffer(poster_config, cover_data, schedule, format="png"):
    ...
    try:
        ...
        # Background line art
        lineart_img = render_background_lineart_layer(
            canvas_width, canvas_height, poster_config, pixels_per_unit
        )
        if lineart_img:
        ...
        title_img = render_text_layer(
            canvas_width,
            canvas_height,
            poster_config.title_text,
            fig_width / 2,
            title_y,
            poster_config.title_fontsize,
            poster_config.title_color,
            poster_config.title_fontfamily,
        ...
        for idx, (date, vols) in enumerate(schedule):
            ...
            date_img = render_text_layer(
                ...
                cell_x,
                date_y,
                poster_config.date_fontsize,
                poster_config.text_color,
                poster_config.title_fontfamily,
                bold=True,
                outline_color=poster_config.background_color,
                outline_width=2,
            ...
            scale_factor = calculate_scale_factor(
                num_vols, frame_instance.width, frame_instance.spacing
            )
            ...
            for j, vol in enumerate(vols):
                total_width = (
                    num_vols * scaled_frame_width
                    + (num_vols - 1) * scaled_frame_spacing
                )
                start_x = cell_x - total_width / 2
                x_center = (
                    start_x
                    + j * (scaled_frame_width + scaled_frame_spacing)
                    + scaled_frame_width / 2
                )
                ...
                # Render frame
                shadow_img, border_img, vertices, mask_img = (
                    frame_instance.render_to_pil(
                        canvas_width,
                        canvas_height,
                        x_center,
                        y_center,
        ...
        if result.mode == "RGBA":
            rgb_result = Image.new(
                "RGB", result.size, hex_to_rgb(poster_config.background_color)
            )
...
def create_poster(poster_config=None, output_path=None, format=None):
    ...
    buffer = render_poster_to_buffer(
        poster_config, merged_covers, schedule, format="png"
    )
    os.makedirs(
